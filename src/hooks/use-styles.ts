...
export const useStyleGuideMoodBoard = (guideImages: MoodBoardImage[]) => {
  ...
  useEffect(() => {
    // Load all the existing images from the server and if we have any client images then we will merge them into this server array
      ...
      if (currentImages?.length === 0) {
        ...
        serverImages.forEach((serverImg) => {
          const clientIndex = mergedImages.findIndex(
            (clientImg) => clientImg.id === serverImg.id
          );
      ...
      for (let i = 0; i < currentImages.length; i++) {
        ...
        if (!image.uploaded && !image.uploading && !image.error) {
          ...
          try {
            ...
            const erroIndex = errorImages.findIndex(
              (img) => img.id === image.id
            );
            if (erroIndex !== -1) {
              errorImages[erroIndex] = {
                ...errorImages[erroIndex],
                uploading: false,
                error: "Upload failed",
              };
